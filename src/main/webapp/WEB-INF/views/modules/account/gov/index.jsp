...
<head>
    ...
    <script>
        ...
        function remindDelete(modelId,key,flag){
            $("#"+modelId).find("#model-ok").attr("href","${ctx}/account/gov/valid/"+key+"/"+flag);
...
<div class="row">
    <div class="col-md-12">
        <ul class="nav nav-tabs">
            <li class="active"><a href="${ctx}/account/finance/index">政府机构列表</a></li>
                    ...
                    <div class="panel-body">
                        <table id="data-table" class="table table-striped table-bordered nowrap" width="100%">
                            ...
                            <c:forEach items="${list}" var="finance" varStatus="idxStatus">
                                    <tr class="unread">
                                        <%--<tr class="odd gradeX">--%>
                                    <td>${finance.accountId}</td>
                                    <td>${finance.account.username}</td>
                                    <td>${finance.name}</td>
                                    <td>${finance.tel}</td>
                                    <td>${finance.areaNames}</td>
                                    <td>${finance.linkman}</td>
                                    <td>${finance.linkmanPhone}</td>
                                    <td><fmt:formatDate value="${finance.createTime}" type="both"/></td>
                                    <td>
                                        <c:if test="${finance.account.valid }">
                                        <a href="javascript:remindDelete('modal-message1','${finance.accountId}',false)">
                                        	<span class="label label-danger">禁用</span>
                                        </a>
                                        </c:if>
                                        <c:if test="${!finance.account.valid}">
											<a href="javascript:remindDelete('modal-message2','${finance.accountId}',true)">
											<span class="label label-success">启用</span>
											</a>
                                        </c:if>
                                        <a href="${ctx}/account/gov/info/${finance.accountId}"><span class="label label-primary">详细</span></a>
									</td>
                                </tr>
                            </c:forEach>
                            </tbody>
